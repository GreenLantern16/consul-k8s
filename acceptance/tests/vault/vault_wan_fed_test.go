...
// This test deploys a single Vault cluster in the primary k8s cluster and exposes to the
...
func TestVault_WANFederationViaGateways(t *testing.T) {
	...
	// Configure a k8s Vault auth method for the second datacenter.
	{
		// Create auth method service account and ClusterRoleBinding. The Vault server
		// in the primary cluster will use this service account's token to talk to the secondary
		// Kubernetes cluster.
		...
		if cfg.UseKind {
			...
			require.NoError(t, err)
			k8sAuthMethodHost = fmt.Sprintf("https://%s:%d", kubernetesEndpoint.Subsets[0].Addresses[0].IP, kubernetesEndpoint.Subsets[0].Ports[0].Port)
		} else {
			k8sAuthMethodHost = helpers.KubernetesAPIServerHostFromOptions(t, secondaryCtx.KubectlOptions(t))
		}
...
func vaultAddress(t *testing.T, cfg *config.TestConfig, ctx environment.TestContext, vaultReleaseName string) string {
	vaultHost := serviceHost(t, cfg, ctx, fmt.Sprintf("%s-vault", vaultReleaseName))
	if cfg.UseKind {
...
func meshGatewayAddress(t *testing.T, cfg *config.TestConfig, ctx environment.TestContext, consulReleaseName string) string {
	primaryMeshGWHost := serviceHost(t, cfg, ctx, fmt.Sprintf("%s-consul-mesh-gateway", consulReleaseName))
	if cfg.UseKind {
...
func serviceHost(t *testing.T, cfg *config.TestConfig, ctx environment.TestContext, serviceName string) string {
	if cfg.UseKind {
		...
		retry.RunWith(&retry.Counter{Wait: 1 * time.Second, Count: 60}, t, func(r *retry.R) {
			vaultSvc, err := ctx.KubernetesClient(t).CoreV1().Services(ctx.KubectlOptions(t).Namespace).Get(context.Background(), serviceName, metav1.GetOptions{})
			require.NoError(t, err)
			require.NotEmpty(r, vaultSvc.Status.LoadBalancer.Ingress)
			// On AWS, load balancers have a hostname for ingress, while on Azure and GCP
			// load balancers have IPs.
			if vaultSvc.Status.LoadBalancer.Ingress[0].Hostname != "" {
				host = vaultSvc.Status.LoadBalancer.Ingress[0].Hostname
			} else {
				host = vaultSvc.Status.LoadBalancer.Ingress[0].IP
			}
		})
		return host
	}
